...
- `didwebs/did.py` — the `WebsDid` value type. Parse/compose/validate per the spec's MSI
  ...
  admits only transferable digest codes — soft spot 3, carried upstream), **plus the refusals
  the ABNF does not make** — a segment that is `.` or `..`, carries `/`, a backslash or
  `\x00`, or begins or ends with whitespace, is refused by name rather than by charset
  (constraint `a2sbz34i`, ported from the sibling `bakobo/webvh-gate`); the **host is
...
1. **Cross-implementation ingest**: artifacts generated by `didwebs` are ingested by the GLEIF
   ...
   correctly. Documented exclusions, not waivers — the authoritative, reasoned list is
   `tests/crossimpl/compare.py`'s `DOCUMENTED_EXCLUSIONS`, printed on every oracle run. It
   began as this design's three (missing `@context`/relationships, the `strip_query` defect,
   clause-0 threshold truncation — KRT-F3, where the reference is wrong and we fail closed)
   and grew two more found reading the reference while building the driver (no top-level
   `controller` at all; mailbox/agent services dropped for non-local AIDs). One suspected
   exclusion was refuted by measurement: the reference ingests our delegated streams cleanly,
   because emission order is delegator-first causal (closed tick 4jke; the self-expiry test
   remains as a regression sensor).
